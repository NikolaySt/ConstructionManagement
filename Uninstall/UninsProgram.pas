...
Const
  ...
  cUnInstKey    =
  	'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Stroitel 2.0';
  C_Path = 'ProgramPath';
...
TryAgeanRegister:
  ...
  DeleteFile(FPath+'\Arhiv\Empty01.zpk');
  DeleteFile(FPath+'\Arhiv\Data01.zpk');
  RemoveDir(FPath+'\Arhiv');
  ProgressBar.Position := ProgressBar.Position + 15;
  Check3.Visible := True;
	Slow;
  ...
  WinDir := StrPas(A);
  DeleteFile(PChar(WinDir+'\DESKTOP\Stroitel 2.0.LNK'));

  DeletePathFiles(WinDir+'\Start Menu\Programs\Stroitel 2.0', '*.*');
  RemoveDir(WinDir+'\Start Menu\Programs\Stroitel 2.0');
  ProgressBar.Position := ProgressBar.Position + 15;
  Check5.Visible := True;
	Slow;
